...
const fetch = async (o) => {
    try {
        ...
        if (o && o.url && (o.url.startsWith("/") || o.url.startsWith(":"))) o.url = baseUrl + o.url;
        const res = await Request({ ...o, headers: o.headers || _headers, url: o.url })
        debug(res, o && o.url && o.url.replace(/\/+$/, '').substring(o.url.lastIndexOf('/') + 1));
        if (res && res.message && res.message.match(/登录已过期|用户未登录/)) throw new Error(`用户需要去登录`);
        return res;
    } catch (e) {
        $.ckStatus = false;
        $.log(`⛔️ 请求发起失败！${e}`);
...
async function main() {
    try {
        //check accounts
        if (!userCookie || !userCookie.length) throw new Error("找不到可用的帐户");
        $.log(`⚙️ 发现 ${userCookie ? userCookie.length : 0} 个帐户\n`);
        let index = 0;
        //doTask of userList
...
async function signin(user) {
    try {
        ...
        for (const activityNo of activityNos) {
            ...
            const reward_num = (res && res.data && res.data.is_popup == 1) ? (res.data.reward_info && res.data.reward_info[0] && res.data.reward_info[0].reward_num) : 0
            results.push(reward_num); // 将每次签到的结果存储到数组中
            $.log(`${$.doFlag[(res && res.data && res.data.is_popup == 1) ? 1 : 0]} ${res && res.data && res.data.is_popup == 1 ? '每日签到: 成功, 获得' + (res.data.reward_info && res.data.reward_info[0] && res.data.reward_info[0].reward_num) + '分' : '每日签到: 今日已签到'}\n`);
...
async function lotterySignin(user) {
    try {
        for (const config of lotteryConfigs) {
            ...
            let res = await fetch(opts);
            $.log(`${$.doFlag[res && res.code == '0000' ? 1 : 0]} ${res && res.code == '0000' ? '抽奖签到: 成功, 获得' + (res.data && res.data.chance) + '次抽奖机会' : '抽奖签到: ' + (res && res.message)}\n`);
            const delaySec = Math.floor(Math.random() * 5) + 1;
            $.log(`⏳ 随机等待 ${delaySec} 秒...`);
            await $.wait(delaySec * 1000);
        }
...
async function lotteryClock(user) {
    try {
        for (const config of lotteryConfigs) {
            ...
            let res = await fetch(opts);
            $.log(`${$.doFlag[res && res.code == '0000' ? 1 : 0]} ${res && res.code == '0000' ? '抽奖成功, 获得' + (res.data && res.data.prize_name) : '抽奖: ' + (res && res.message)}\n`);
            const delaySec = Math.floor(Math.random() * 5) + 1;
            $.log(`⏳ 随机等待 ${delaySec} 秒...`);
            await $.wait(delaySec * 1000);
    }
...
async function getUserInfo(user) {
    try {
        ...
        let res = await fetch(opts);
        let growth_value = (res && res.data && res.data.growth_value) || 0;
        $.log(`🎉 ${(res && res.code == '0000') ? '您当前成长值: ' + growth_value : (res && res.message)}\n`);
        return res && res.data
    } catch (e) {
        $.log(`⛔️ 查询用户信息失败！${e}\n`)
    }
...
async function getBalance(user) {
    try {
        ...
        let res = await fetch(opts);
        let balance = (res && res.data && res.data.balance) || 0;
        let expiring_lz = (res && res.data && res.data.expiring_lz) || 0;
        $.log(`🎉 ${(res && res.code == '0000') ? '您当前珑珠: ' + balance + ', 即将过期: ' + expiring_lz : (res && res.message)}\n`);
        return res && res.data
    } catch (e) {
        $.log(`⛔️ 查询用户珑珠失败！${e}\n`)
    }
...
async function getLotteryConfigs(user) {
    try {
        ...
        let res = await fetch(opts);
        if (res && res.data && res.data.components) {
...
async function Request(t) { "string" == typeof t && (t = { url: t }); try { if (!t || !t.url) throw new Error("[发送请求] 缺少 url 参数"); let { url: o, type: e, headers: r = {}, body: s, params: a, dataType: n = "form", resultType: u = "data" } = t; const p = e ? e.toLowerCase() : "body" in t ? "post" : "get", c = o.concat("post" === p ? "?" + $.queryStr(a) : ""), i = t.timeout ? $.isSurge() ? t.timeout / 1e3 : t.timeout : 1e4; "json" === n && (r["Content-Type"] = "application/json;charset=UTF-8"); const y = s && "form" == n ? $.queryStr(s) : $.toStr(s), l = { ...t, ...(t && t.opts ? t.opts : {}), url: c, headers: r, ..."post" === p && { body: y }, ..."get" === p && a && { params: a }, timeout: i }, m = $.http[p.toLowerCase()](l).then((t => "data" == u ? $.toObj(t.body) || t.body : $.toObj(t) || t)).catch((t => $.log(`❌请求发起失败！原因为：${t}`))); return Promise.race([new Promise(((t, o) => setTimeout((() => o("当前请求已超时")), i))), m]) } catch (t) { console.log(`❌请求发起失败！原因为：${t}`) } }

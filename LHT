        ...
        for (const activityNo of activityNos) {
            const opts = {
                ...
                headers: {
                    ...
                    'X-LF-DXRisk-Captcha-Token': user['x-lf-dxrisk-captcha-token'] || 'undefined',
                    'X-LF-DXRisk-Source': user['x-lf-dxrisk-source'] || '5',

                    // 业务头
                    'X-GAIA-API-KEY': user['x-gaia-api-key'],
                    'X-LF-Bu-Code': user['x-lf-bu-code'] || 'C20400',
                    'X-LF-Channel': user['x-lf-channel'] || 'C2',
                    ...
                    // UA
                    'User-Agent': user['user-agent'] || 'Mozilla/5.0 (iPhone; CPU iPhone OS 26_5_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.75(0x18004b45) NetType/4G Language/zh_CN miniProgram/wx50282644351869da',
            ...
            $.log(`📊 HTTP状态: ${res.http_status_code || 'unknown'}\n`);

            // 检查 401
            if (res.http_status_code === 401 || res.message === "Unauthorized") {
                $.log(`❌ 认证失败! 可能原因:\n`);
                $.log(`  1. token 已过期: ${user.token}\n`);
                $.log(`  2. DXRisk-Token 无效: ${user['x-lf-dxrisk-token']}\n`);
                $.log(`  3. Cookie 已失效: ${user.cookie}\n`);
                $.log(`  请重新抓包获取最新的认证信息\n`);
                continue;
            }
            ...
            if (res.code && res.code !== "0000") {
                $.log(`❌ 业务错误! code: ${res.code}, message: ${res.message}\n`);
                continue;
            }
            ...
            } else if (res?.data?.is_popup == 0) {
                $.log(`ℹ️ 今日已签到\n`);
            } else {
                $.log(`⚠️ 未知状态: ${JSON.stringify(res.data)}\n`);
            }
...
async function Request(t) { "string" == typeof t && (t = { url: t }); try { if (!t?.url) throw new Error("[发送请求] 缺少 url 参数"); let { url: o, type: e, headers: r = {}, body: s, params: a, dataType: n = "form", resultType: u = "data" } = t; const p = e ? e?.toLowerCase() : "body" in t ? "post" : "get", c = o.concat("post" === p ? "?" + $.queryStr(a) : ""), i = t.timeout ? $.isSurge() ? t.timeout / 1e3 : t.timeout : 1e4; "json" === n && (r["Content-Type"] = "application/json;charset=UTF-8"); const y = s && "form" == n ? $.queryStr(s) : $.toStr(s); const { type: _, ...rest } = t; const l = { ...rest, ...t?.opts ? t.opts : {}, url: c, headers: r, ..."post" === p && { body: y }, ..."get" === p && a && { params: a }, timeout: i }; let m; if ($.isNode()) { const got = require("got"); const requestPromise = got[p](l).then((t => { const { statusCode: r, headers: a, body: o } = t; return "data" == u ? $.toObj(o) || o : $.toObj(o) || o })).catch((t => $.log(`❌请求发起失败！原因为：${t}`))); m = requestPromise; } else { m = $.http[p.toLowerCase()](l).then((t => "data" == u ? $.toObj(t.body) || t.body : $.toObj(t) || t)).catch((t => $.log(`❌请求发起失败！原因为：${t}`))); } return Promise.race([new Promise(((t, o) => setTimeout((() => o("当前请求已超时")), i))), m]) } catch (t) { console.log(`❌请求发起失败！原因为：${t}`) } }
